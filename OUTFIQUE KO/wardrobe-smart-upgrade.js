class WardrobeApp {
  ...
  bindWardrobeEvents() {
    ...
    document.getElementById("openCameraBtn")?.addEventListener("click", () => this.openCameraModal());
    ...
    document.getElementById("savePhotoBtn")?.addEventListener("click", () => this.savePhotoToWardrobe());
  }
  ...
  resetCameraUI() {
    ...
    document.querySelector(".camera-overlay")?.classList.remove("active");
    document.getElementById("capturePreview")?.classList.add("hidden");
    document.getElementById("clothingForm")?.classList.add("hidden");
  ...
  async startCamera() {
    ...
    try {
      ...
      this.showCameraMessage("Camera access denied. Please allow camera permissions.", "error");
    }
  }
  ...
  capturePhoto() {
    const video = document.getElementById("cameraFeed");
    if (!video.classList.contains("active")) {
      this.showCameraMessage("Please start camera first.", "error");
      return;
    }
  ...
  usePhoto() {
    document.getElementById("capturePreview").classList.add("hidden");
    document.getElementById("clothingForm").classList.remove("hidden");
  }

  async savePhotoToWardrobe() {
    const category = document.getElementById("cameraCategory")?.value;
    const style = document.getElementById("cameraStyle")?.value || "modern";
    const description = document.getElementById("cameraDescription")?.value.trim() || "Captured clothing item";

    if (!category) {
      this.showCameraMessage("Please select a category.", "error");
      return;
    }

    try {
      const response = await this.apiRequest("/api/item/create", "POST", {
        category,
        style,
        description,
      ...
      if (!response.success) {
        this.showCameraMessage(response.error || "Failed to save item.", "error");
        return;
      }

      this.showCameraMessage("✓ Item saved successfully!", "success");
      setTimeout(() => {
      ...
      this.showCameraMessage(error.message || "Failed to save item.", "error");
    }
  }

  showCameraMessage(message, type = "") {
